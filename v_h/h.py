...
def register(key):
    try:
        ...
        start_time_dict = {'1': '59/56.0', '2': '59/56.5', '3': '59/57.0', '4': '59/56.8',
                           '5': '59/56.0', '6': '59/56.3', '7': '59/57.0', '8': '59/55.7'}
        ...
        for i in range(3):
            ...
            telegram.send_doc(caption=f'{name}-{key}слотHserg Не прогрузился язык или дата', html=driver.page_source)
            raise RuntimeError(f'Не прогрузился язык или дата {name}-{key}слотHserg')
        ...
        logging.warning(f'Ввод имя: {name}-{key}слотHserg')
        ...
        logging.warning(f'Нажали выбор даты-{key}слотHserg:{dt}')
        if f.is_element_displayed('//span[text()="Свободно"]'):
            ...
            telegram.send_doc(f'В. Даты {name}-{key}слотHserg, {start_time_dict[key]}', source)
            sleep(90)
            telegram.send_doc(f'В. Перед завершением бронирования {name}-{key}слотHserg', driver.page_source)
            f.click_on_while('Завершение бронирования')
            dt = datetime.strptime(datetime.now(tz=timezone.utc).strftime('%m/%d/%Y/%H/%M/%S.%f'),
                                   '%m/%d/%Y/%H/%M/%S.%f')
            logging.warning(f'ЗАПИСАН:({name}): {dt}')
            sleep(10)
            telegram.send_doc(f'🟩В: в {dt} успешно зарегистрирован({name}-{key}слотHserg {start_time_dict[key]})',
                              driver.page_source)
        else:
            if f.is_element_displayed(
                ...
                telegram.send_doc(f'⭕В {name}-{key}слотHserg уже зареген другим сеансом {start_time_dict[key]}',
                                  driver.page_source)
        ...
        try:
            telegram.send_image(driver, f'В неизвестная ошибка-{key}слотHserg {str(e)} {start_time_dict[key]}')
        except Exception:
            telegram.send_message(f'В неизвестная ошибка. -{key}слотHserg {str(e)}')
