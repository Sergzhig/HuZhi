...
def register(dt):
    try:
        ...
        logging.warning(f'Ввод имя: {name}{index}слот{t}Hserg{user}')
        ...
        logging.warning(f'Нажали выбор даты:{dt}{index}слот{t}Hserg{user}')
        if f.is_element_displayed('//span[text()="Свободно"]'):
            ...
            sleep(90)
            telegram.send_doc(f'В. Перед завершением бронирования {name} {index}слот{t}Hserg{user}', driver.page_source)
            f.click_on_while('Завершение бронирования')
            dt = datetime.strptime(datetime.now(tz=timezone.utc).strftime('%m/%d/%Y/%H/%M/%S.%f'),
                                   '%m/%d/%Y/%H/%M/%S.%f')
            logging.warning(f'ЗАПИСАН:({name}): {dt}')
            sleep(10)
            telegram.send_doc(f'🟩В: в {dt} успешно зарегистрирован({name} {index}слот{t}Hserg{user})',
                              driver.page_source)
        else:
            if f.is_element_displayed(
                ...
                telegram.send_doc(f'⭕В {name} уже зареген другим сеансом {index}слот{t}Hserg{user}',
                                  driver.page_source)
                logging.warning('Уже зареген')
                driver.close()
            else:
                telegram.send_doc(f'⭕В для:{name} нет дат {index}слот{t}Hserg{user}', driver.page_source)
                logging.warning(f'Нет дат: {index}слот{t}Hserg{user}')
        ...
        try:
            telegram.send_image(driver, f'В неизвестная ошибка {str(e)} {index}слот{t}Hserg{user}')
        except Exception:
            telegram.send_message(f'В неизвестная ошибка.{index}слот{t}Hserg{user} {str(e)}')
